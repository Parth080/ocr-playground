...
import { getExtractAsyncStatus } from '../lib/api'
import PollingStatus from '../components/PollingStatus'
import ResponsePanel from '../components/ResponsePanel'

export default function ExtractAsyncStatusPage() {
  ...
  async function handleFetch() {
    ...
    setError(null)
    try {
      const res = await getExtractAsyncStatus(statusId)
      setData(res)
    } catch (e) {
      setError(e.message)
    } finally {
  ...
  const startPolling = useCallback(() => {
    ...
    const poll = async () => {
      try {
        setPollCount((c) => c + 1)
        const status = await getExtractAsyncStatus(statusId)
        setData(status)
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <code className="text-sm text-gray-400 font-mono">/extract-async/&#123;status_id&#125;</code>
        </div>
        <h1 className="text-2xl font-bold text-white">Poll Extract Status</h1>
        <p className="text-gray-400 mt-1">Check the processing status of an extraction job.</p>
      </div>
      ...
      <div className="card mb-6">
        ...
        <input
          ...
          placeholder="Enter the record_id from extract-async"
